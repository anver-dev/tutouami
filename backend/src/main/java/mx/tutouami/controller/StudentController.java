...
public class StudentController {
	...
	public ResponseEntity<List<StudentDTO>> retrieveAll(
			@ApiParam(name = "Authorization", value = "Bearer token", example = Seguridad.HEADER_AUTORIZACION, required = false) 
			@RequestHeader(value = "Authorization", name = "Authorization", required = false) String authorization) {

		//if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
			//List<StudentDTO> alumnos = studentService.findAll();
		//}
	 ...
	 * @return
	 * @ApiOperation(value = "Elimina alumno", notes = "Elimina a un alumno mediante
	 *                     su ID")
	 * @ApiResponses(value = { @ApiResponse(code = 200, message = "Alumno eliminado
	 *                     exitosamente"),
	 * @ApiResponse(code = 404, message = "No se encontro el alumno"),
	 * @ApiResponse(code = 500, message = "Error al eliminar al alumno") })
	 * @DeleteMapping(path = "/alumnos/{idAlumno}") public ResponseEntity<?> delete(
	 * @ApiParam(name = "Authorization", value = "Bearer token", example =
	 *                Seguridad.HEADER_AUTORIZACION, required =
	 *                true) @RequestHeader(value = "Authorization", name =
	 *                "Authorization", required = true) String
	 *                authorization, @PathVariable("idAlumno") @Valid Long idAlumno)
	 *                {
	 * 
	 *                log.info("Buscando al alumno con id para eliminarlo: " +
	 *                idAlumno);
	 * 
	 *                try { if
	 *                (servicioSeguridad.jwtEsValido(authorization.replace("Bearer
	 *                ", ""))) { StudentDTO alumno =
	 *                studentService.retrieve(idAlumno); if (alumno != null) {
	 *                studentService.delete(idAlumno); return
	 *                ResponseEntity.status(HttpStatus.OK).body("Alumno eliminado
	 *                correctamente");
	 * 
	 *                } else return
	 ...
	 *                ResponseEntity.status(HttpStatus.NOT_FOUND).body("No se
	 *                encontro alumno"); }
	 * 
	 *                }
	 */
	 ...
	 * @return
	 * @ApiOperation(value = "Actualiza el perfil de un alumno", notes = "Se
	 *                     actualiza el alumno")
	 * @ApiResponses(value = { @ApiResponse(code = 200, message = "Alumno
	 *                     actualizado exitosamente"),
	 * @ApiResponse(code = 404, message = "No se encontro al alumno para
	 *                   actualizar"),
	 * @ApiResponse(code = 500, message = "Error en el servidor") })
	 * @PatchMapping(path = "/alumnos/{idAlumno}", consumes =
	 ...
	 *                true) @RequestHeader(value = "Authorization", name =
	 *                "Authorization", required = true) String
	 *                authorization, @PathVariable("idAlumno") Long
	 *                idAlumno, @RequestBody @Valid StudentDTO alumnoDto) {
	 * 
	 ...
	 *                (servicioSeguridad.jwtEsValido(authorization.replace("Bearer
	 *                ", ""))) { StudentDTO alumno =
	 *                studentService.actualizarAlumno(idAlumno, alumnoDto);
	 * 
	 ...
	 *                HttpStatus.NOT_FOUND; else status =
	...
	public ResponseEntity<?> retrieveAll(@PathVariable("idAlumno") Long idAlumnoAbuscar,
		...
		try {
			if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
				return ResponseEntity.status(HttpStatus.CREATED).body(studentService.findById(idAlumnoAbuscar));
			}
	...
	public ResponseEntity<AdviceDTO> create(
			@ApiParam(name = "Authorization", value = "Bearer token", example = Seguridad.HEADER_AUTORIZACION, required = true) @RequestHeader(value = "Authorization", name = "Authorization", required = true) String authorization,
			@RequestBody @Valid AdviceDTO nuevaAsesoria, @PathVariable("id") Long id) {
		try {

			if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
				AdviceDTO asesoriaDto = servicioAsesoria.agregaAsesoria(nuevaAsesoria, id);
	...
	public ResponseEntity<?> delete(
		...
		try {
			...
			if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
				AdviceDTO asesoria = servicioAsesoria.buscarID(idAsesoria);
				if (asesoria != null) {
					...
					if (servicioAsesoria.eliminarAsesoria(asesoria))
						...
						return ResponseEntity.status(HttpStatus.CONFLICT)
								.body("No se puede eliminar la asesoria ya que cuenta con inscripciones");
				} else {
	...
	public ResponseEntity<AdviceDTO> update(
			@ApiParam(name = "Authorization", value = "Bearer token", example = Seguridad.HEADER_AUTORIZACION, required = true) @RequestHeader(value = "Authorization", name = "Authorization", required = true) String authorization,
			@PathVariable("idAlumno") Long idAlumno, @PathVariable("idAsesoria") Long idAsesoria,
			@RequestBody @Valid AdviceDTO asesoriaDto) {
		...
		try {
			if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
				AdviceDTO asesoria = servicioAsesoria.actualizar(idAsesoria, idAlumno, asesoriaDto);
	...
	public ResponseEntity<?> update(
		...
		try {
			if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
				Optional<InscriptionDTO> opAsesoria = servicioInscripcion.actualizarEstado(idInscripcion, idAlumno,
						estado);
	...
	public ResponseEntity<List<AdviceDTO>> retrieve(
		...
		if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
			List<AdviceDTO> asesoriasDto = servicioAsesoria.recuperaAsesorias(idAlumno);
	...
	public ResponseEntity<List<InscriptionDTO>> retrieveAll(
			@ApiParam(name = "Authorization", value = "Bearer token", example = Seguridad.HEADER_AUTORIZACION, required = true) @RequestHeader(value = "Authorization", name = "Authorization", required = true) String authorization,
			@PathVariable("idAlumno") Long idAlumno) {
		...
		try {
			if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
				...
				List<InscriptionDTO> inscripciones = servicioInscripcion.recuperaInscripcionesPorAlumno(idAlumno);
	...
	public ResponseEntity<InscriptionDTO> create(
			@ApiParam(name = "Authorization", value = "Bearer token", example = Seguridad.HEADER_AUTORIZACION, required = true) @RequestHeader(value = "Authorization", name = "Authorization", required = true) String authorization,
			@PathVariable("idAlumno") Long idAlumno, @RequestBody AdviceDTO asesoriaDto) {

		log.info("Empezando HU-01");
		try {
			if (servicioSeguridad.jwtEsValido(authorization.replace("Bearer ", ""))) {
				InscriptionDTO alumnoDto = servicioInscripcion.agregaInscripcion(asesoriaDto.getIdAsesoria(), idAlumno);
